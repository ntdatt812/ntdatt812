...
[![Open](https://img.shields.io/badge/open_for_review-63_PRs-8b949e?style=flat-square&labelColor=161b22)](https://github.com/ntdatt812)
...
| Pull requests open | **63** | Awaiting maintainer review |
...
**[diegosouzapw/OmniRoute](https://github.com/diegosouzapw/OmniRoute)** — AI gateway, 55.2k★.
...
| PR | What it does |
| --- | --- |
| [#3691](https://github.com/nicolargo/glances/pull/3691) | `WifiPlugin.get_alert` chained its three threshold comparisons, so the first level the config left undefined was compared against `None`, raised `TypeError`, and the blanket handler dropped the whole result to `DEFAULT`. Since `critical` is tested first, a config setting only `wifi_careful` produced no wifi alert at all — silently. The base class already guards each level separately; wifi was the one override that did not. |
| [#3690](https://github.com/nicolargo/glances/pull/3690) | The RAID WebUI decided degradation with one bare comparison while `raid_alert` applies three rules before it. A raid0 array was drawn as degraded — it has no redundancy, and the TUI excludes it in two places — and so was an array whose device count could not be read, because `null < 5` is true in JavaScript. Four of seven arrays disagreed between the two views before; none after. |
...
| [#3687](https://github.com/nicolargo/glances/pull/3687) | The `CPU*` summary row sliced `percpu_list[0 : max_cpu_display]` — the same slice `msg_curse` had just printed. It averaged the cores already on screen instead of the ones that did not fit, and since that list is sorted busiest-first when it overflows, it systematically over-reported: 87.0% for a group of cores sitting at 2.0%. |
