...
Public data for [`ntdatt812`](https://github.com/ntdatt812), counted **2026-09-22**,
...
| [thedotmack/claude-mem](https://github.com/thedotmack/claude-mem) | 94.2k★ | 4 open. [#4189](https://github.com/thedotmack/claude-mem/pull/4189) — the sanitizer that keeps screenshots out of the observer matches on the shape of a content block, so it needs parsed JSON, and it is never given any: the ingest boundary stores a tool payload as JSON text. It had therefore never run on a real observation, only in tests that build the field themselves. A 300k-character frame reached the prompt and was also sent to a model to be summarised. Also [#4190](https://github.com/thedotmack/claude-mem/pull/4190), where the supervisor registry overwrote a still-running process's record, and nothing signals a pid that is not in the map. [#3619](https://github.com/thedotmack/claude-mem/pull/3619) adds the tests `main` lacks for the double-append fix that landed upstream, and [#3620](https://github.com/thedotmack/claude-mem/pull/3620) warns when a reply is cut off at the output-token limit and names the cap. |
