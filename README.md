...
[![Landed](https://img.shields.io/badge/landed-82_PRs_%2B_6_commits-d97757?style=flat-square&labelColor=161b22)](https://github.com/ntdatt812)
...
[![Open](https://img.shields.io/badge/open_for_review-107_PRs-8b949e?style=flat-square&labelColor=161b22)](https://github.com/ntdatt812)
...
Public data for [`ntdatt812`](https://github.com/ntdatt812), counted **2026-09-09**,
...
not counted. The figures are the sum of `gh pr list --repo <repo> --author ntdatt812
--state all`, run against each repository in turn, rather than `gh search prs`: the
search index lags by hours, and the last time I compared the two it disagreed with the
per-repository counts in both directions.

| | Count | What it counts |
| --- | ---: | --- |
| Pull requests merged | **82** | Merged by maintainers of repos I don't own |
...
| Pull requests open | **107** | Awaiting maintainer review |
...
— [jdx/mise](https://github.com/jdx/mise), 33.3k★. Moderate, CVSS 4.0 **5.9**.
...
**[github/spec-kit](https://github.com/github/spec-kit)** — spec-driven development toolkit, 133.7k★.
...
**[farion1231/cc-switch](https://github.com/farion1231/cc-switch)** — provider switcher for Claude Code and Codex, 131.8k★.
...
**[diegosouzapw/OmniRoute](https://github.com/diegosouzapw/OmniRoute)** — AI gateway, 61.9k★.
Nineteen pull requests merged.

| PR | What it does |
| --- | --- |
| [#11573](https://github.com/diegosouzapw/OmniRoute/pull/11573) | The semantic cache wrote `expires_at` as an ISO string and read it back against SQLite's `datetime('now')`. Both are TEXT, and they diverge at index 10 where one has `T` and the other a space, so every entry whose UTC calendar date was today compared as unexpired no matter what its TTL said — up to ~24h stale, and it survived restarts (closes [#11559](https://github.com/diegosouzapw/OmniRoute/issues/11559)). |
| [#11574](https://github.com/diegosouzapw/OmniRoute/pull/11574) | `/v1/models` passed Next's `after()` to a function that had stopped taking it, so the stale-while-revalidate rebuild ran before the response was flushed and the caller paid for the rebuild it was supposed to be spared (closes [#11551](https://github.com/diegosouzapw/OmniRoute/issues/11551)). |
| [#11582](https://github.com/diegosouzapw/OmniRoute/pull/11582) | A lazy-registry refactor made `getExecutor()` async and left the conformance gate calling it synchronously. It reported all 142 aliases dead — and while it printed that number the check was in fact blind, unable to tell a real breakage from the noise. |
| [#11580](https://github.com/diegosouzapw/OmniRoute/pull/11580) | Replaces a `size === 1` pin on a security-relevant allowlist with exact membership, which also catches a substitution the count could not. |
| [#11588](https://github.com/diegosouzapw/OmniRoute/pull/11588), [#11576](https://github.com/diegosouzapw/OmniRoute/pull/11576), [#11577](https://github.com/diegosouzapw/OmniRoute/pull/11577), [#11583](https://github.com/diegosouzapw/OmniRoute/pull/11583) | The rest of the gates that were red on every branch at once, each from its own cause: a packaging guard a same-day merge never registered; an env var left out of the docs contract; two covering test files missing from the mutation-coverage config; and a bundler assertion pinned to the documented contract rather than to a reading of it. |
| [#11589](https://github.com/diegosouzapw/OmniRoute/pull/11589), [#11590](https://github.com/diegosouzapw/OmniRoute/pull/11590), [#11591](https://github.com/diegosouzapw/OmniRoute/pull/11591), [#11592](https://github.com/diegosouzapw/OmniRoute/pull/11592), [#11593](https://github.com/diegosouzapw/OmniRoute/pull/11593) | The unit-test failures underneath those gates — no two sharing a cause. Five `__MISSING__:` markers rendering as literal text in the Vietnamese UI; a model alias whose premise half-expired when two of four catalogs moved to the bare id; an assertion still demanding a truncation that had been removed for paralysing subagents, in direct contradiction of the test that owns that contract; a terminal-skip boundary that moved when expired connections were given retry budget; and a Gemini alias retarget. |
...
| [#12177](https://github.com/diegosouzapw/OmniRoute/pull/12177) | The combos editor dropped a cleared field from the `PUT` body, and the update merges over the stored record — so an omitted field means "leave unchanged". Unchecking context cache protection never persisted and the toggle came back on. `description` and `context_length` were already cleared with an explicit `null` in the same function. |
| [#12180](https://github.com/diegosouzapw/OmniRoute/pull/12180) | A self-hosted embedding endpoint was absent from the registry that supplies vector widths, so both write paths deadlocked on the resulting `null`: the store refused to create `vec_memories` without a width, and the width could only come from an embedding neither path would make. Memories piled up unvectorized behind a green health check. |

**[tinyhumansai/openhuman](https://github.com/tinyhumansai/openhuman)** — local-first AI agent desktop app, 39.5k★.
Six pull requests merged.

| PR | What it does |
| --- | --- |
| [#5821](https://github.com/tinyhumansai/openhuman/pull/5821) | The tool-policy boundary was prepended to the system prompt, where the model reads it before the instructions it is meant to bound. Appended, it lands after them. |
| [#5817](https://github.com/tinyhumansai/openhuman/pull/5817) | Finishes the per-config MCP connection lookups — half the call sites still resolved against a single global connection, so a second configured server was reachable only by accident of ordering. |
| [#5815](https://github.com/tinyhumansai/openhuman/pull/5815) | Removes `omit_skills_catalog`, a prompt flag nothing had read since the catalogue moved: 16 call sites across the crate plus 11 files under `tests/raw_coverage/` that `cargo check --lib --tests` never compiles. |
...
**[can1357/oh-my-pi](https://github.com/can1357/oh-my-pi)** — coding agent with the IDE wired in, 29.8k★.
...
**[decolua/9router](https://github.com/decolua/9router)** — LLM API router, 27.2k★.
...
**[h4ckf0r0day/obscura](https://github.com/h4ckf0r0day/obscura)** — headless browser engine in Rust, 26.1k★.
Six pull requests merged.

| PR | What it does |
| --- | --- |
| [#741](https://github.com/h4ckf0r0day/obscura/pull/741) | `HTMLInputElement.indeterminate` did not exist, so the tri-state checkbox every table's select-all header uses read back as plain `false`. It also had to be cleared on activation, per HTML's legacy-pre-activation step — in both copies of that logic, since the CDP mouse path carries its own. The issue text described the clear as *replacing* the checked toggle; it is an additional step, and the maintainer confirmed the issue was wrong rather than my reading. |
| [#745](https://github.com/h4ckf0r0day/obscura/pull/745) | An MCP tool replied before a synthesized navigation had drained, so an agent was told a submit click had happened while the process had not yet left the page. |
| [#748](https://github.com/h4ckf0r0day/obscura/pull/748) | The screenshot warm-up fetched every entry of a `@font-face` `src` list. That list is a priority order, not a set — the renderer already read it correctly, and only the warm-up did not, so a page with four fallbacks paid for four downloads to use one. |
| [#743](https://github.com/h4ckf0r0day/obscura/pull/743) | A one-line feature gate on a test that broke the no-render build. The first plan — gate only the paint-dependent assertion — compiled and then failed, because without `paint` the "loaded" face *is* the fallback face and both sides of the `assert_ne!` measure the same font. Trading a compile error for a failing test is worse than the bug, so the whole test is gated instead. |
| [#747](https://github.com/h4ckf0r0day/obscura/pull/747) | A rejected promise came back from `Runtime.callFunctionOn` as an ordinary result, so `page.evaluate(() => Promise.reject(new Error('x')))` resolved to `{}` instead of throwing, and `Promise.reject({code: 42})` was byte-identical to resolving with it. Reported and fixed rather than picked up from the tracker. |
...
**[lidge-jun/opencodex](https://github.com/lidge-jun/opencodex)** — coding agent, 13.6k★.
...
**[nicolargo/glances](https://github.com/nicolargo/glances)** — system monitor, 33.5k★.
Twenty-two pull requests merged.

| PR | What it does |
| --- | --- |
| [#3699](https://github.com/nicolargo/glances/pull/3699) | The process list coloured the `MEM%` cell by passing `cpu_percent` to the `processlist_mem_` thresholds, so the memory column was decorated from the process's CPU usage. |
| [#3698](https://github.com/nicolargo/glances/pull/3698) | The per-CPU table coloured the `iowait` cell by passing `cpu.system` to the `percpu_iowait_` thresholds, the same shape one column over. |
| [#3697](https://github.com/nicolargo/glances/pull/3697) | `plugin-vms.vue` decided the VM status colour with one inline comparison against `stopped`, while `VmsPlugin.vm_alert()` knows three more states. `starting`, `restarting` and `delayed shutdown` are WARNING in the plugin and were drawn as OK in the WebUI, and `stopped` is INFO rather than CAREFUL. |
| [#3696](https://github.com/nicolargo/glances/pull/3696) | `plugin-amps.vue` computed its own decoration rather than reading `AmpsPlugin.get_alert()`, which the TUI calls directly. An out-of-range AMP count came out CAREFUL (blue) where the plugin says WARNING (purple). |
| [#3695](https://github.com/nicolargo/glances/pull/3695) | The ports component disagreed with the ports plugin on two counts, both verified against a live `/api/4/ports` response rather than inferred: `status` arrives as a boolean, a number or a string, and `rtt_warning` defaults to `null`. |
| [#3701](https://github.com/nicolargo/glances/pull/3701) | `_port_scan_tcp` set its socket timeout with `socket.setdefaulttimeout()`, which is process-wide, and never restored it. After one scan every socket created anywhere in Glances that does not set its own timeout inherited the last scanned port's value: measured on `develop`, hddtemp's socket came back with `3.0` where it had been `None`. Scopes the timeout to the scanning socket. |
| [#3700](https://github.com/nicolargo/glances/pull/3700) | `load_limits` split comma-separated config values without stripping, so `list=cpu, mem, load` became a list whose last two items carry a leading space. Those items are used as `re.fullmatch` patterns, so a leading space matches nothing at all, and `conf/glances.conf` documents the spaced form in its own comment. Anyone copying the shipped example got a silently dead `hide=` or `show=` rule. |
| [#3694](https://github.com/nicolargo/glances/pull/3694) | The network plugin skipped `get_alert` when *either* direction measured 0 bytes/s, because the guard was `or`. A send-only interface saturating its uplink went undecorated for having an rx rate of exactly 0, and a fully idle one read `DEFAULT` instead of `OK`. 0 is a measurement, not a missing value; diskio had already dropped the same guard in #3684. |
| [#3691](https://github.com/nicolargo/glances/pull/3691) | `WifiPlugin.get_alert` chained its three threshold comparisons, so the first level the config left undefined was compared against `None`, raised `TypeError`, and the blanket handler dropped the whole result to `DEFAULT`. Since `critical` is tested first, a config setting only `wifi_careful` produced no wifi alert at all — silently. |
| [#3690](https://github.com/nicolargo/glances/pull/3690) | The RAID WebUI decided degradation with one bare comparison while `raid_alert` applies three rules before it, so a raid0 array was drawn as degraded (it has no redundancy; the TUI excludes it) and an unreadable device count was too, because `null < 5` is true in JavaScript. Four of seven arrays disagreed between the two views before, none after. |
| [#3688](https://github.com/nicolargo/glances/pull/3688) | The sensor alert loop skipped on `not i['value']`, so a reading of exactly 0 was thrown away with the absent ones. The battery alert is computed on `100 - value`, which made an empty battery the one reading that never raised an alert, while 3% was CRITICAL. |
| [#3687](https://github.com/nicolargo/glances/pull/3687) | The `CPU*` summary row sliced `percpu_list[0 : max_cpu_display]`, the same slice `msg_curse` had just printed, so it averaged the cores already on screen rather than the ones that did not fit. That list is sorted busiest-first when it overflows, so the row systematically over-reported: 87.0% for a group of cores sitting at 2.0%. |
| [#3684](https://github.com/nicolargo/glances/pull/3684) | The disk I/O alert was computed from `read_bytes`, the cumulative counter since boot, so it crossed any threshold once and stayed there forever. The network plugin next door already alerts on the rate. Alerts on the bitrate, and decorates the field the WebUI actually reads. |
| [#3683](https://github.com/nicolargo/glances/pull/3683) | The connections alert was raised against a constant 0 instead of the tracked-connection percentage, so a conntrack table filling up never warned. |
| [#3681](https://github.com/nicolargo/glances/pull/3681) | The `?` placeholder for an unreadable value was only produced for `undefined` and the empty string, and the API sends `null` for a nice value the OS will not report, which rendered as an empty cell instead. |
| [#3678](https://github.com/nicolargo/glances/pull/3678) | With several container engines running, a fragment of the title was repeated once per engine. |
| [#3677](https://github.com/nicolargo/glances/pull/3677) | The ICMP timeout was passed in seconds to a ping helper expecting milliseconds, so healthy remote hosts timed out and read as offline. |
| [#3676](https://github.com/nicolargo/glances/pull/3676) | The per-folder refresh timer was guarded by `i in self.timer_folders`, where `i` is a list index and the list holds `Timer` objects. `Timer` has no `__eq__`, so the test was always false and every folder was walked on every cycle — `folder_N_refresh` did nothing. Bounds-checks the index instead. |
| [#3675](https://github.com/nicolargo/glances/pull/3675) | `_sum_stats` accumulated with `sub_key in p[key]`. That is a mapping test, which is right for `memory_info` but wrong for `io_counters`, where `sub_key` is a list index — so `0 in p['io_counters']` asked whether the counters happened to contain the number zero. Indexes instead of testing membership. |
| [#3674](https://github.com/nicolargo/glances/pull/3674) | `GlancesFilter.filter` split the input on every colon and kept only the second field, silently discarding everything after the second one. A filter regex containing a colon was therefore truncated. Splits once, on the first colon. |
...
**[soxoj/maigret](https://github.com/soxoj/maigret)** — username OSINT across 3,000+ sites, 37.3k★.
...
| --- | --- | --- |
| [github/spec-kit](https://github.com/github/spec-kit) | 134.2k★ | 5 open. [#4460](https://github.com/github/spec-kit/pull/4460) — the condition gate answered "does every operand resolve?" by re-parsing the expression itself, a second implementation of the evaluator's grammar kept in step by hand. It now reports the leaves the evaluator actually reached. Also [#4295](https://github.com/github/spec-kit/pull/4295), the hole #4292 closed one construct over; [#4313](https://github.com/github/spec-kit/pull/4313), which counts checkbox markers outside code fences only; [#4314](https://github.com/github/spec-kit/pull/4314), which scopes issue dedup to the feature the tasks belong to; and [#4330](https://github.com/github/spec-kit/pull/4330), which stops `converge` re-appending work an unchecked task already tracks. |
| [garrytan/gstack](https://github.com/garrytan/gstack) | 132.2k★ | [#2636](https://github.com/garrytan/gstack/pull/2636) — the `GITHUB_` prefix admitted operator credentials into hermetic child environments. |
| [farion1231/cc-switch](https://github.com/farion1231/cc-switch) | 131.8k★ | 8 open. [#6477](https://github.com/farion1231/cc-switch/pull/6477) — Codex Desktop's `[desktop]` config table was wiped on every provider switch. Also [#6474](https://github.com/farion1231/cc-switch/pull/6474), [#6476](https://github.com/farion1231/cc-switch/pull/6476) and [#6479](https://github.com/farion1231/cc-switch/pull/6479); [#6928](https://github.com/farion1231/cc-switch/pull/6928), where the Codex live-write plan is decided by one notion of which provider is official and the switch that follows it by another, so the plan can promise a write the switch will not make; [#7010](https://github.com/farion1231/cc-switch/pull/7010), which points `session_summary` at the selected model profile; [#7187](https://github.com/farion1231/cc-switch/pull/7187), a zh-TW wording nit returned by the maintainer; and [#7193](https://github.com/farion1231/cc-switch/pull/7193), which drops `baseUrl` so the config survives TypeScript 7. |
| [thedotmack/claude-mem](https://github.com/thedotmack/claude-mem) | 93.5k★ | 16 open. [#3619](https://github.com/thedotmack/claude-mem/pull/3619) — the provider recorded every assistant reply into the conversation history twice, and nothing dedupes it before it becomes the request's `messages` array, so the assistant half of every later request was double-billed. Also [#3727](https://github.com/thedotmack/claude-mem/pull/3727), which stops the worker daemon holding the user's project directory as its cwd, and [#3739](https://github.com/thedotmack/claude-mem/pull/3739), where the `discovery_tokens` repair returns early on a row in `schema_versions` — a record that the migration once ran, not that the column is still there — so once anything rebuilds the table in its old shape the repair can never run again. Also [#3740](https://github.com/thedotmack/claude-mem/pull/3740), [#3726](https://github.com/thedotmack/claude-mem/pull/3726), [#3620](https://github.com/thedotmack/claude-mem/pull/3620), [#3593](https://github.com/thedotmack/claude-mem/pull/3593). Then six on the observer, each one a way a batch is lost rather than retried: [#3786](https://github.com/thedotmack/claude-mem/pull/3786) reads the CLI's signed-out prose as an auth failure, [#3759](https://github.com/thedotmack/claude-mem/pull/3759) requeues a batch when the child reports a transport failure, [#3760](https://github.com/thedotmack/claude-mem/pull/3760) pauses the session on a reactive quota error rather than ending it, [#3762](https://github.com/thedotmack/claude-mem/pull/3762) keeps image payloads out of the observation prompt, [#3757](https://github.com/thedotmack/claude-mem/pull/3757) guards `discovery_tokens` on the column itself, [#3756](https://github.com/thedotmack/claude-mem/pull/3756) stops an unparseable Bash command blocking the tool call, and [#3808](https://github.com/thedotmack/claude-mem/pull/3808) stops a slow attempt being retried against a deadline it has already spent. Newest: [#3810](https://github.com/thedotmack/claude-mem/pull/3810), where FTS5's `unicode61` tokenizer has no delimiter to split Chinese or Japanese on, so a whole run of ideographs is one token and no substring of it — which is every query in those scripts — can ever match; and [#3811](https://github.com/thedotmack/claude-mem/pull/3811), which fits the session context block to the host's 10,000-character hook limit by dropping whole items rather than by counting them, since an item's size varies enough that no fixed count is both safe and useful. |
| [diegosouzapw/OmniRoute](https://github.com/diegosouzapw/OmniRoute) | 63k★ | 16 open. [#12896](https://github.com/diegosouzapw/OmniRoute/pull/12896) — the in-memory call-log filter is narrower than the SQL `WHERE` it re-applies, so it silently drops rows the query just returned. [#12930](https://github.com/diegosouzapw/OmniRoute/pull/12930) masks PII inside a `tool_result`'s nested content, which the redactor walked past while still reporting success. [#12925](https://github.com/diegosouzapw/OmniRoute/pull/12925) accepts the 16th argument `glm.ts` has been passing since #12179 — that arity error fails the API-typecheck gate on every branch in the repository, and the 64 KB stream buffer its call site asks for has never taken effect. [#13009](https://github.com/diegosouzapw/OmniRoute/pull/13009) fixes the skills-sync gate, red on every branch for the same kind of reason: the registry parser cannot see positionals declared with `.addArgument()`. Also [#12918](https://github.com/diegosouzapw/OmniRoute/pull/12918), [#12920](https://github.com/diegosouzapw/OmniRoute/pull/12920), [#12921](https://github.com/diegosouzapw/OmniRoute/pull/12921), [#12932](https://github.com/diegosouzapw/OmniRoute/pull/12932), [#13007](https://github.com/diegosouzapw/OmniRoute/pull/13007), [#13024](https://github.com/diegosouzapw/OmniRoute/pull/13024) and [#13025](https://github.com/diegosouzapw/OmniRoute/pull/13025). [#13083](https://github.com/diegosouzapw/OmniRoute/pull/13083) accepts a null `dailyQuotaResetTimezone`: the dashboard sends both quota-reset fields as null, and only the hour's schema allowed it, so an edit touching neither still failed validation. [#13087](https://github.com/diegosouzapw/OmniRoute/pull/13087) stops the dashboard's model health test probing a Responses-typed provider node on /v1/chat/completions: the route can answer 200 while carrying nothing a Chat Completions reader recognises, so those models were marked dead while the same model answered normally on /v1/responses. Two on the request scanner, each measured rather than argued: [#13101](https://github.com/diegosouzapw/OmniRoute/pull/13101) reads the text a `tool_result` block carries on `content` — the extractor only ever read `text`, so nothing a tool handed back was scanned at all, which is the one carrier whose bytes come from outside the conversation; and [#13104](https://github.com/diegosouzapw/OmniRoute/pull/13104) spends the 16 KB scan budget on both ends of the body rather than the first 16 KB, because the extractor appends `system`, `instructions`, `query` and `documents` after the message list, so one long message hid all four of them and the newest turns with them. [#13110](https://github.com/diegosouzapw/OmniRoute/pull/13110) came out of a scan for lists of the same thing written down twice: the tool-schema sanitiser knew ten of the draft 2020-12 keywords whose value is a subschema, the opencode plugin's own walker knew twelve, and a truncation placeholder in either of the two it was missing was forwarded to the provider as a string — which is the 400 that sanitiser exists to prevent. |
| [tinyhumansai/openhuman](https://github.com/tinyhumansai/openhuman) | 39.5k★ | 20 open. [#5586](https://github.com/tinyhumansai/openhuman/pull/5586) — `rpcUrl` credentials were written to the log unredacted; [#5583](https://github.com/tinyhumansai/openhuman/pull/5583) adds a lint rule for the config boundary the frontend documents but never enforced. Then a run at the Claude Code stream reader, where separate paths lose bytes: [#5718](https://github.com/tinyhumansai/openhuman/pull/5718) decodes stdout across chunk boundaries, [#5980](https://github.com/tinyhumansai/openhuman/pull/5980) bounds the stderr buffer without splitting a character, and [#5741](https://github.com/tinyhumansai/openhuman/pull/5741) reports the unparsable lines the parser deliberately keeps and the event mapper silently drops. Also [#5744](https://github.com/tinyhumansai/openhuman/pull/5744), which stops `subagentStop` being reported as wired when nothing fires it, and [#5777](https://github.com/tinyhumansai/openhuman/pull/5777), which flags the subagents `run_subagent` will not dispatch. [#5822](https://github.com/tinyhumansai/openhuman/pull/5822) drops both current-user caches on sign-out; [#5747](https://github.com/tinyhumansai/openhuman/pull/5747) lets a profile named outside ASCII be saved; [#5790](https://github.com/tinyhumansai/openhuman/pull/5790) launches `claude auth login` rather than the `claude login` the CLI no longer accepts. [#5794](https://github.com/tinyhumansai/openhuman/pull/5794) reports Claude's structured error instead of an empty stderr, [#5816](https://github.com/tinyhumansai/openhuman/pull/5816) stops assistant roles being emitted on the CLI's stdin, and [#5834](https://github.com/tinyhumansai/openhuman/pull/5834) stops the approval-gate tests racing the TTL they park under. [#5762](https://github.com/tinyhumansai/openhuman/pull/5762) makes `clippy -D warnings` pass on a Windows host, [#5769](https://github.com/tinyhumansai/openhuman/pull/5769) installs the memory seams the `clear_memory` tests relied on another test to have installed, and [#5982](https://github.com/tinyhumansai/openhuman/pull/5982) fixes a lint that reads English out of the translation catalogues as Tailwind classes — every Windows push fails on it. [#5792](https://github.com/tinyhumansai/openhuman/pull/5792) / [#5793](https://github.com/tinyhumansai/openhuman/pull/5793) correct the README's Rust version and point Windows contributors at the script that works. [#6064](https://github.com/tinyhumansai/openhuman/pull/6064) waits for the loaded sandbox panel rather than for the load to start — the suite's `waitFor(mock was called)` already held on its first tick, while the panel was still rendering its loading paragraph. [#6057](https://github.com/tinyhumansai/openhuman/pull/6057) drops `baseUrl` so `app/tsconfig` survives TypeScript 7. |
