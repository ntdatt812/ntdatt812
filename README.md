...
| PR | What it does |
| --- | --- |
| [#3726](https://github.com/thedotmack/claude-mem/pull/3726) | The Windows credential lookup compiled a C# type `ClaudeMem.CredRead` with a method also named `CredRead`, which C# rejects (CS0542). Two separate `SilentlyContinue` settings hid the compile error, so the script exited 0 with empty output, which the caller reads as "Credential Manager has no entry" — a wrong answer on a machine that has the credential, which is why it went unnoticed. Renames the method and marks a shim failure so it can no longer read as an absent credential (closes [#3722](https://github.com/thedotmack/claude-mem/issues/3722)). |
| [#3786](https://github.com/thedotmack/claude-mem/pull/3786) | When a reboot signed the profile out, the CLI's `Not logged in · Please run /login` matched none of the observer's auth-failure patterns, which all key on the word *authentication*. Each batch was logged as a prose response, confirmed and lost: 857 in three hours on one reporter's machines, with no observations written. The signed-out wording is now an auth failure. |
| [#3810](https://github.com/thedotmack/claude-mem/pull/3810) | FTS5's `unicode61` tokenizer has no delimiter to split Chinese or Japanese on, so a whole run of ideographs is one token and no substring of it — which is every query in those scripts — could ever match. CJK queries now fall back to substring matching. |
| [#4189](https://github.com/thedotmack/claude-mem/pull/4189) | The sanitizer that keeps inlined images out of the observer matches on the shape of a content block, so it needs parsed JSON — and it was never given any. The ingest boundary stores a tool payload as JSON *text*, the provider encodes it a second time, and the prompt builder parses once, so the sanitizer was always handed a string and returned it untouched. It had therefore never run on a real observation; the tests that covered it build the field themselves, single-encoded, which is the one form production never produces. A 300k-character video frame reached the prompt and, because the condense pass runs first, was also sent to a model to be summarised. Reported spend was 225k-501k input tokens per call. Also strips the shape Claude Code's Read returns for an image file, which has no `source` key and so matched no branch even once the field was parsed. |
| [#4190](https://github.com/thedotmack/claude-mem/pull/4190) | The supervisor registry keyed processes by a caller-supplied id, and some are fixed for the life of the product — chroma always registers under `chroma-mcp` — so each new generation's `register()` replaced the previous generation's record while that process was still running. Nothing signals a pid that is not in the map: shutdown walks the registry and the dead-entry sweep only visits entries. A displaced record whose process is still alive is now re-keyed rather than dropped, which puts it back in front of the reapers that already exist. |
| [#3727](https://github.com/thedotmack/claude-mem/pull/3727) | A hook-spawned worker inherited the hook's working directory, which is the user's project. A process holds its cwd open for its whole life, and on Windows that locks the folder against rename and move for as long as the daemon runs, which outlives the session that started it (closes [#3706](https://github.com/thedotmack/claude-mem/issues/3706)). Every launch path now starts the worker in the claude-mem data directory, created first, since a missing cwd makes spawn fail with `ENOENT`. |
...
| [#12925](https://github.com/diegosouzapw/OmniRoute/pull/12925) | `glm.ts` had been passing a 16th argument — a 64 KB stream buffer — to a function that takes fifteen, ever since #12179. The arity error failed the API-typecheck gate on every branch in the repository, and the buffer size its call site asked for had never taken effect. |
| [#13009](https://github.com/diegosouzapw/OmniRoute/pull/13009) | The skills-sync gate was red on every branch for the same kind of reason: the registry parser could not see positionals declared with `.addArgument()`, so the committed registry and a regenerated one always disagreed — and regenerating to make them agree would have deleted real parameters. Teaches the parser that form instead. |
| [#12896](https://github.com/diegosouzapw/OmniRoute/pull/12896) | The call-log view re-applies its SQL filter in memory, and the in-memory predicate was narrower than the `WHERE` clause it copies, so it silently dropped rows the query had just returned. |
| [#12930](https://github.com/diegosouzapw/OmniRoute/pull/12930) | The PII redactor walked past text inside a `tool_result`'s nested `content` array, and still reported the request as redacted. |
| [#13101](https://github.com/diegosouzapw/OmniRoute/pull/13101) | The injection scanner's extractor only ever read a block's `text`, and a `tool_result` carries its text on `content` — so nothing a tool handed back was scanned at all, which is the one carrier whose bytes come from outside the conversation. |
| [#13104](https://github.com/diegosouzapw/OmniRoute/pull/13104) | The scanner's 16 KB budget was spent on the first 16 KB of the body. The extractor appends `system`, `instructions`, `query` and `documents` after the message list, so one long message hid all four of them and the newest turns with them. Spends the budget on both ends. |
| [#13110](https://github.com/diegosouzapw/OmniRoute/pull/13110) | Came out of a scan for lists of the same thing written down twice: the tool-schema sanitiser knew ten of the draft 2020-12 keywords whose value is a subschema, the opencode plugin's own walker knew twelve, and a truncation placeholder in either of the two it was missing was forwarded to the provider as a string — which is the 400 that sanitiser exists to prevent. |
| [#12918](https://github.com/diegosouzapw/OmniRoute/pull/12918) | `/api/a2a/status` built its agent card by calling the `/.well-known/agent.json` handler in-process with no request, so the card fell back to a hardcoded `localhost:20128`. A gateway reached at any other host — LAN address, reverse proxy, container name — advertised a URL no one else could reach. Forwards the status route's own request. |
| [#12920](https://github.com/diegosouzapw/OmniRoute/pull/12920) | Context aging prepended its annotation to any turn with no text block to replace. A user turn answering a `tool_use` with a single `tool_result` is exactly that shape, and the Messages API requires `tool_result` to lead the turn, so upstream rejected the whole request. Appends the annotation for that shape and leaves every other ordering as it was. |
| [#12921](https://github.com/diegosouzapw/OmniRoute/pull/12921) | Bedrock ids are normalised before the context-limit lookup, and the vendor peel was hardcoded to `anthropic.`, so every `openai.*` model was stored with no limit and fell back to 200k — a 1M-context model was refused locally before the request ever reached AWS. |
| [#13007](https://github.com/diegosouzapw/OmniRoute/pull/13007) | Azure's `max_tokens` → `max_completion_tokens` swap matched a literal `gpt-5`, so each new generation arrived broken and `gpt-6-astra` failed every request. Matches the generation range instead — deliberately not `gpt-\d+`, which would also catch Azure's own `gpt-35-turbo` and strip a parameter that deployment needs (closes [#12981](https://github.com/diegosouzapw/OmniRoute/issues/12981)). |
| [#13083](https://github.com/diegosouzapw/OmniRoute/pull/13083) | The dashboard sends both quota-reset fields as `null`, and only the hour's schema allowed it, so an edit touching neither still failed validation on `dailyQuotaResetTimezone`. |
...
| [#5834](https://github.com/tinyhumansai/openhuman/pull/5834) | The five approval-gate expiry tests each parked under a two-second TTL read from the process environment while other tests rewrote that variable, so a test could expire early or late depending on scheduling. Each now holds the environment lock only until its park has captured the TTL, then restores the prior value. |
| [#6057](https://github.com/tinyhumansai/openhuman/pull/6057) | TypeScript 7 drops `baseUrl`, and `app/tsconfig.json` still set it. Removing it needed proof rather than a delete: the bare imports it was resolving had to be checked against the `paths` entries that survive, and the real build run, because Vite reads the config too. |
...
| [#5816](https://github.com/tinyhumansai/openhuman/pull/5816) | The Claude Code transport wrote assistant-role lines onto the CLI's stdin, which rejects any line whose role is not `user` and exits 1 before the model is ever called. |
| [#5793](https://github.com/tinyhumansai/openhuman/pull/5793) | The README sent Windows contributors to the POSIX dev script; the repository ships a Windows one. |
| [#5792](https://github.com/tinyhumansai/openhuman/pull/5792) | The README named a Rust version the toolchain file does not pin. |
| [#5790](https://github.com/tinyhumansai/openhuman/pull/5790) | The settings panel launched `claude login`. There is no such subcommand, and because the CLI takes a positional prompt it does not error — it runs the word as a prompt, OAuth never starts, and the panel reports the user as signed out forever. |
| [#5777](https://github.com/tinyhumansai/openhuman/pull/5777) | `agent.list_subagents` advertised subagents that `run_subagent` refuses to dispatch. The refusal condition and the listing now read one predicate, so the catalogue cannot drift from what dispatch accepts. |
| [#5762](https://github.com/tinyhumansai/openhuman/pull/5762) | `clippy -D warnings` failed on a Windows host with eleven errors in `#[cfg(windows)]` branches the Linux CI never compiles, which meant the pre-push hook could not pass from Windows at all. |
| [#5747](https://github.com/tinyhumansai/openhuman/pull/5747) | A profile whose name fell outside ASCII could not be saved. |
| [#5744](https://github.com/tinyhumansai/openhuman/pull/5744) | The hooks panel reported `subagentStop` as wired when nothing fires it. |
| [#5741](https://github.com/tinyhumansai/openhuman/pull/5741) | The stream parser deliberately keeps the lines it cannot parse, and the event mapper dropped them, so a malformed frame left no trace anywhere. Reports the line's shape and byte count rather than its content, because the repository forbids logging payloads. |
| [#5718](https://github.com/tinyhumansai/openhuman/pull/5718) | The stdout reader decoded each chunk on its own, so a multi-byte character split across a chunk boundary was corrupted. |
...
| PR | What it does |
| --- | --- |
| [#3725](https://github.com/nicolargo/glances/pull/3725) | `hide_zero` hid an idle interface or disk for exactly one refresh and then stopped. The shipped configuration says a rate *less or equal than* `hide_threshold_bytes` stays hidden, but the comparison un-hid on `>=`, so at the documented default of 0 every zero rate satisfied it. |
| [#3726](https://github.com/nicolargo/glances/pull/3726) | The network plugin never read the `hide_threshold_bytes` its own documentation offers and diskio has always read. |
| [#3727](https://github.com/nicolargo/glances/pull/3727) | The WebUI and `msg_curse` read the same per-field `hide_zero` verdict in opposite directions — the browser dropped a row when any field was hidden, the terminal kept it while any was visible — so a disk that had only ever been read was shown in one and missing from the other. |
| [#3721](https://github.com/nicolargo/glances/pull/3721) | `compute_rate_on_list` skipped a stat that was new since the previous sample, so the next reader of it raised `KeyError`. |
| [#3722](https://github.com/nicolargo/glances/pull/3722) | The four VM columns `msg_curse` requested decorations for were never assigned one. |
| [#3724](https://github.com/nicolargo/glances/pull/3724) | Colours a container's CPU and MEM cells in the WebUI the way curses already did, reading the per-container threshold the server had already applied. |
| [#3718](https://github.com/nicolargo/glances/pull/3718) | Both interfaces asked for an NPU temperature alert that nothing computed. |
| [#3717](https://github.com/nicolargo/glances/pull/3717) | Each DISK I/O column in the WebUI was coloured by the other direction's rate. |
| [#3716](https://github.com/nicolargo/glances/pull/3716) | Each `--percpu` bar is now coloured by that core's own load. |
