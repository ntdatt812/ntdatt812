...
[![Landed](https://img.shields.io/badge/landed-36_PRs_%2B_6_commits-d97757?style=flat-square&labelColor=161b22)](https://github.com/ntdatt812)
...
| --- | ---: | --- |
| Pull requests merged | **36** | Merged by maintainers of repos I don't own |
...
| [diegosouzapw/OmniRoute](https://github.com/diegosouzapw/OmniRoute) | 55.2k★ | 15 open. [#11573](https://github.com/diegosouzapw/OmniRoute/pull/11573) — the semantic cache wrote `expires_at` as an ISO string and read it back against SQLite's `datetime('now')`. Both are TEXT, and they diverge at index 10 where one has `T` and the other a space, so every entry whose UTC calendar date was today compared as unexpired no matter what its TTL said — up to ~24h stale, and it survived restarts. Also [#11574](https://github.com/diegosouzapw/OmniRoute/pull/11574), where `/v1/models` passed Next's `after()` to a function that had stopped taking it, so the stale-while-revalidate rebuild ran before the response was flushed and the caller paid for the rebuild anyway. The other four are the CI gates that were red on every branch at once, each from a different cause: [#11582](https://github.com/diegosouzapw/OmniRoute/pull/11582) — a lazy-registry refactor made `getExecutor()` async and left the conformance gate calling it synchronously, so all 142 aliases were reported dead and the check could no longer tell a real breakage from the noise; [#11580](https://github.com/diegosouzapw/OmniRoute/pull/11580) replaces a `size === 1` pin on a security-relevant allowlist with exact membership, which also catches a substitution the count could not; [#11576](https://github.com/diegosouzapw/OmniRoute/pull/11576) documents an env var a same-day refactor left out of the contract; and [#11577](https://github.com/diegosouzapw/OmniRoute/pull/11577) registers two covering test files the mutation-coverage gate reported as missing. [#11586](https://github.com/diegosouzapw/OmniRoute/pull/11586) is the same refactor's third casualty — a guard pinning `"claude-web": new ClaudeWebExecutor()` by spelling, where what it protects is which class the entry builds. [#11583](https://github.com/diegosouzapw/OmniRoute/pull/11583) is the one I would not send without asking: the assertion and the code disagree about whether Bun forces webpack, so it pins the documented contract and says plainly which way to invert it if I read the evidence wrong. The last one, [#11587](https://github.com/diegosouzapw/OmniRoute/pull/11587), took the longest to find because the job that fails prints nothing at all: `No new ESLint warnings` was not over any warning budget, it was exiting 2 because two bulk-suppression entries described problems the code had stopped producing. The remaining six are the unit-test failures underneath those gates, and no two share a cause: a packaging guard #11437 never registered ([#11588](https://github.com/diegosouzapw/OmniRoute/pull/11588)); five `__MISSING__:` markers rendering as literal text in the Vietnamese UI ([#11589](https://github.com/diegosouzapw/OmniRoute/pull/11589)); a model alias whose premise half-expired when two of four catalogs moved to the bare id ([#11590](https://github.com/diegosouzapw/OmniRoute/pull/11590)); an assertion still demanding the 128-tool truncation that #11444 removed for paralysing subagents, in direct contradiction of the test that owns that contract ([#11591](https://github.com/diegosouzapw/OmniRoute/pull/11591)); a terminal-skip boundary that moved when expired connections were given retry budget ([#11592](https://github.com/diegosouzapw/OmniRoute/pull/11592)); and #11503's Gemini alias retarget ([#11593](https://github.com/diegosouzapw/OmniRoute/pull/11593)). |
...
| [nicolargo/glances](https://github.com/nicolargo/glances) | 33.4k★ | 7 open. [#3691](https://github.com/nicolargo/glances/pull/3691) — `WifiPlugin.get_alert` chained its three threshold comparisons, so the first level the config left undefined was compared against `None`, raised `TypeError`, and the blanket handler dropped the whole result to `DEFAULT`. Since `critical` is tested first, a config setting only `wifi_careful` produced no wifi alert at all — silently. [#3690](https://github.com/nicolargo/glances/pull/3690) — the RAID WebUI decided degradation with one bare comparison while `raid_alert` applies three rules before it, so a raid0 array was drawn as degraded (it has no redundancy; the TUI excludes it) and an unreadable device count was too, because `null < 5` is true in JavaScript. Four of seven arrays disagreed between the two views before, none after. [#3687](https://github.com/nicolargo/glances/pull/3687) — the `CPU*` summary row sliced `percpu_list[0 : max_cpu_display]`, the same slice `msg_curse` had just printed, so it averaged the cores already on screen rather than the ones that did not fit. Since that list is sorted busiest-first when it overflows, the row systematically over-reported: 87.0% for a group of cores sitting at 2.0%. [#3688](https://github.com/nicolargo/glances/pull/3688) — the sensor alert loop skipped on `not i['value']`, so a reading of exactly 0 was thrown away with the absent ones; the battery alert is computed on `100 - value`, which made an empty battery the one reading that never raised an alert while 3% was CRITICAL. Then the WebUI pass: [#3682](https://github.com/nicolargo/glances/pull/3682) colours the per-CPU iowait cell from iowait, [#3681](https://github.com/nicolargo/glances/pull/3681) shows `?` for a nice value the OS will not report, and [#3680](https://github.com/nicolargo/glances/pull/3680) carries the Windows priority labels into the WebUI. |
